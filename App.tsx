...
export default function App() {
  ...
  const initializeDatabase = async () => {
    try {
      ...
      await db.executeSql('CREATE TABLE IF NOT EXISTS tasks (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, assignedTo TEXT, project_id INTEGER);');
  ...
  const addProject = () => {
    if (!db) return;
    db.executeSql('INSERT INTO projects (title) VALUES (?);', [newProjectTitle]).then(([result]) => {
  ...
  const addTask = () => {
    if (!db) return;
    db.executeSql('INSERT INTO tasks (name, assignedTo, project_id) VALUES (?, ?, ?);', [newTaskName, taskAssignee, selectedProject.id]).then(() => {
      setNewTaskName('');
      setTaskAssignee('');
    ...
    <>
      ...
      <View style={styles.container}>
        ...
        {(role === 'Employee' || role === 'Manager') && selectedProject && (
          <View>
            <Text style={{ marginBottom: 10, fontWeight: 'bold', fontSize: 18, color: '#D9534F' }}>Project: {selectedProject.title}</Text>
            {role === 'Manager' && (
            ...
            <FlatList
              ...
              keyExtractor={(item) => item.id.toString()}
              renderItem={({ item }) => (
                <Text style={{ padding: 10, backgroundColor: '#eee', marginTop: 5 }}>{item.name} (Assigned to: {item.assignedTo})</Text>
              )}
            />
          </View>
        )}
        ...
        <Modal visible={showTaskModal} animationType="slide">
          <View style={styles.container}>
            ...
            />
            <Text style={{ marginVertical: 10, fontWeight: 'bold', fontSize: 18, color: '#D9534F' }}>Assign To:</Text>
            <Picker
              selectedValue={taskAssignee}
              onValueChange={(value) => setTaskAssignee(value)}>
              <Picker.Item label="Select Employee" value="" />
              {(projectMembers[selectedProject?.id] || []).map((emp) => (
